...
/*
  Admin add: three explicit image fields (photo1, photo2, photo3)
*/
...
export default function AddChurch() {
  ...
  const submit = async (e: any) => {
    ...
    if (photos.length > 3) { alert('Maximum 3 photos autorisées'); return; }
    setLoading(true);
    try {
      const urls: string[] = [];
      ...
      const profile = await supabase.from('profiles').select('id,is_approved').eq('auth_id', user?.id).single();
      if (!profile.data || !profile.data.is_approved) { alert('Compte non approuvé'); setLoading(false); return; }
  ...
  return (
    <div style={{ maxWidth: 920, margin: '40px auto', padding: 20 }}>
      ...
      <form onSubmit={submit} style={{ display: 'grid', gap: 12 }}>
        ...
        <div>
          <button className="btn-primary" disabled={loading} type="submit">{loading ? 'Envoi...' : 'Ajouter'}</button>
        </div>
      </form>
    </div>
  );
